...
import TheLibrarian.cards.SBBullet;
...
public class BulletTracker extends AbstractPower implements CloneablePowerInterface {
    public AbstractCreature source;

    public static final String POWER_ID = TheLibrarianMod.makeID("Friendship");
    ...
    public BulletTracker(final AbstractCreature owner, final AbstractCreature source, final int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public void onCardDraw(AbstractCard card) {
        if (card.cardID == SBBullet.ID) {
            flash();
            if(amount==6){
                addToBot(new DamageAction(owner, new DamageInfo(owner, 35, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.BLUNT_HEAVY));
                amount=0;
            }else{
                amount++;
                addToBot(new DamageAllEnemiesAction(null,
                        DamageInfo.createDamageMatrix(this.amount*5, true), DamageInfo.DamageType.THORNS, AbstractGameAction.AttackEffect.FIRE, true));
            }
        }
    }
    ...
    public AbstractPower makeCopy() {
        return new BulletTracker(owner, source, amount);
    }
}
