...
public class ScarsPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public ScarsPower(final AbstractCreature owner, final AbstractCreature source, final int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public void wasHPLost(DamageInfo info, int damageAmount) {
        if (info.owner != null && info.owner != this.owner && info.type != DamageInfo.DamageType.HP_LOSS && info.type != DamageInfo.DamageType.THORNS && damageAmount > 0) {
            flash();
            addToBot(new ApplyPowerAction(owner, owner, new NextTurnBlockPower(owner, this.amount), this.amount));
    ...
    public AbstractPower makeCopy() {
        return new ScarsPower(owner, source, amount);
    }
}
